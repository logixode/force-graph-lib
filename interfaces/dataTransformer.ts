...
import { FullResponse, Pagination } from "./graphResponse";
...
export interface DataTransformer {
   ...
   */
  transformData(rawData: FullResponse<Pagination>, platformColors: Record<string, string>): GraphData;
}
...
export class DefaultDataTransformer implements DataTransformer {
  transformData(rawData: FullResponse<Pagination>, platformColors: Record<string, string>): GraphData {
    ...
    if (Array.isArray(data.nodes)) {
      ...
      result.nodes = data.nodes.map((node: any) => ({
        ...node,
      ...
      if (Array.isArray(data.data)) {
        result.links = data.data.map((link: any) => ({
          ...link,
            ...
            Object.entries(platformNodes).forEach(([topic, nodesItem]) => {
              if (nodesItem) {
                Object.entries(nodesItem).forEach(([nodeKey, node]) => {
                  const label = `${(node as any).label ?? (node as any).title} (${nodeKey == "0" ? 'Topic' : (node as any).type})`
                  const nodeData = {
                    ...node,
                    label,
                    color:
                      nodeKey == "0" ? "#000000" : (node as any).color || platformColor,
                    platform,
                    topic,
                    // x:0,
                    // y:0,
            ...
            Object.entries(platformEdges).forEach(([topic, edgesItem]) => {
              if (edgesItem) {
                Object.values(edgesItem).forEach((edge) => {
                  if (edge && (edge as any).from && (edge as any).to) {
                    const linkData = {
                      source: (edge as any).from,
                      target: (edge as any).to,
                      color: (edge as any).color,
                      platform,
                      topic,
                    };
