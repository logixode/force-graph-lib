import ForceGraphRenderer, { LinkObject } from "force-graph";
import * as d3 from "d3";
import { GraphData, GraphOptions, NodeData, LinkData } from "../interfaces/types";
...
export class ForceGraph {
  ...
  private initGraph() {
    ...
    this.graph.nodeCanvasObject(
      (node, ctx: CanvasRenderingContext2D, globalScale: number) => {
  ...
  private applyOptions() {
    if (this.options.layout === "circlepack") {
      ...
      if (this.options.noCollision) {
        // Force-directed layout with no collision
        this.graph.d3Force("collide", null); // Remove collision force
      }
    }
  }
  ...
  private applyCirclePackLayout() {
    ...
    const simulation = d3
      ...
      .force("charge", d3.forceManyBody<NodeData>().strength(0))
      .force(
        "collide",
        d3
          .forceCollide<NodeData>()
          .radius((d: NodeData) => this.getNodeSize(d) * 1.2)
      )
      .force("x", d3.forceX<NodeData>().strength(0.1))
      .force("y", d3.forceY<NodeData>().strength(0.1));

    this.graph.d3Force("charge", null);
    this.graph.d3Force("link", null);
    this.graph.d3Force("center", null);
    this.graph.d3Force("collide", simulation.force("collide") ?? null);
    this.graph.d3Force("x", simulation.force("x") ?? null);
    this.graph.d3Force("y", simulation.force("y") ?? null);
  }
  ...
  public async addData(newData: GraphData): Promise<void> {
    ...
    // Use a web worker for layout calculations if available
